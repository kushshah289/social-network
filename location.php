...
//echo $_GET['loc'];
	...
	$comm = "select * from comments inner join profile on profile.user_id=comments.commentor WHERE comments.Status_c=1 ORDER by comments.comment_id";
       ...
       if ($result2->num_rows > 0) {
        ...
        $sql1 = "select  concat(profile.firstname,' ', profile.lastname) as Name,profile.firstname, profile. lastname, diary.post_id, date_format(diary.created_date,'%M %D %Y') AS cr_date, diary.user_id,diary.diary_title ,diary.description,diary.created_date,diary.multimedia, diary.location, profile.picture AS picture from diary inner join profile on diary.user_id = profile.user_id where diary.location = '$_GET[loc]' and diary.Status_d=1"; 
