...
<?php
$query="select picture from profile where user_id=$_SESSION[user_id]";
...
<?php
$query="select firstname,date_format(birthdate,'%D %M, %Y') as birthdate,email,country,lastname,country,fteam,fsport,fplayer,picture from profile where user_id=$_SESSION[user_id]";
$result=mysqli_query($con,$query);
if(mysqli_num_rows($result)>0)
{
    echo "<h3 style=text-decoration:underline><strong>About me:  </strong>  </h3> ";
...
$(document).on('click','.like-icon', function(){
    ...
    $(document).ready(function(){
        ...
        //alert(multimedia_file_display);
            var html="";
            html += "<div class='main-container' id='post_"+diaries[key]['post_id']+"'><div class='name_title'><div class='name'><a href='friendsprofile.php?id="+diaries[key]['user_id']+"'><h6><strong>"+diaries[key]['Name']+"</strong></h6></a><img src='uploads/"+diaries[key]['picture']+"' width=30 height=30/></div><div class='title'><span><h3>"+diaries[key]['diary_title']+"</h3><h6 class='date'>"+diaries[key]['cr_date']+"</h6><h5>"+(diaries[key]['location']!==""?"<span class='glyphicon glyphicon-map-marker'></span><a href='http://localhost/project/location.php?loc="+diaries[key]['location']+"'<span class='location'>"+diaries[key]['location']+"</span></a>":"")+"</h5><strong><span post_id='"+diaries[key]['post_id']+"' class='glyphicon glyphicon-remove remove-post' style='float:right;'>Delete</span></strong></div></div><div class='desc_comment'><div class='description'><h5>"+diaries[key]['description']+"</h5></span>"+multimedia_file_display+"</div><div class='likes_block'>Likes:<span id='like_block_"+diaries[key]['post_id']+"'></span><span class='like_symbol' id='like_symbol_"+diaries[key]['post_id']+"'></span></div><div id='comments_"+diaries[key]['post_id']+"' class='commments'><h6>----comments here----</h6></div></div></div>";
